...
wrapper <- function(idx) {
  ...
  #for(signal_i in 1:4){
    ...
    res=validation_broadcasted_sparsetenreg(R,alpha,lambda,X_data[,,id_train],y_all[id_train],X_data[,,id_vali],y_all[id_vali],X_data[,,id_test],y_all[id_test],num_knots=5)
    BB_idx[[1]]=res$BB
    BB_idx[[1+4]]=res$bb
...
sfSource('./ParallelComput/parallel_source_500_butterfly.R')
...
sfStop()
filename <- str_c("BNTR", n_use, "_butterfly_K6.Rdata")
setwd("./SimResults")
