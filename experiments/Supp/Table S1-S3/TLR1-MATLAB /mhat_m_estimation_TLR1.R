...
# load the data set
if(isbutterfly == 0){
  loadname = str_c("./SimResults/TLR1_", ndata, str_c("_linear_new_", distri,".Rdata"))
}else if(isbutterfly == 1){
  # 默认就是原始的uniform setting的结果
  loadname = str_c("./SimResults/TLR1_", ndata, str_c("_linear_butterfly",".Rdata"))
}
...
if(isbutterfly==0){
  savename= str_c('TLR1_res_mat', ndata, str_c("_linear_new_",distri,".Rdata"))
}else if(isbutterfly==1){
  savename= str_c('TLR1_res_mat', ndata, str_c("_linear_butterfly_",".Rdata"))
}
