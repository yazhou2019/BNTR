#parallel sourece
library(stringr)
library(rTensor)
library(glmnet)
library(Matrix)
library(MASS)

source('./ComponentsNonLin/functions_needed.R')
source('./ComponentsNonLin/broadcasted_sparsetenreg.R')
source('./ComponentsNonLin/sequential_warmstart.R')
source('./ComponentsNonLin/validation_broadcasted_sparsetenreg.R')
source('./SimDataGeneration/true_signal_different.R')
source('./SimDataGeneration/data_generation_different.R')

# 
# R=c(1,2,3,4,5)
# alpha=c(0,0.5,1)
# lambda=c(0.01,0.05,0.1,0.5,1,5,10,50,100,500,1000)


n_use=500

#zhao的文章是对每一个样本都估计一个均值和方差
#新讨论，那就用极大似然原理来估计（zhao的文章并没有说明用什么方法估计）
#再次讨论，用所有样本来估计总的协方差矩阵。均值就用每个样本值来代替
VU_est <- function(Xmat){
  
  pm = dim(Xmat)[1]
  p_m = dim(Xmat)[2]
  if(length(dim(Xmat)) > 1){
    n = dim(Xmat)[3]
  }else{
    n=1
  }
  
  X_mean = matrix(0,pm,p_m)
  
  for(i in 1:pm){
    for(j in 1:p_m){
      X_mean[i,j] = mean(Xmat[i,j,])
    }
  }
  for(i in 1:n){
    Xmat[,,i] = Xmat[,,i] - X_mean
  }
  
  set.seed(2008)
  V0 = matrix(rnorm(p_m*p_m),p_m,p_m)
  U = 0
  U_new = 0
  
  V=V0
  
  for(iter in 1:100){
    Vinv = ginv(V)
    for(i in 1:n){
      U_new = U_new + Xmat[,,i]%*%Vinv%*%t(Xmat[,,i])
    }
    U_new = U_new/(p_m*n)
    Uinv = ginv(U_new)
    V_new = 0
    for(i in 1:n){
      V_new = V_new + t(Xmat[,,i])%*%Uinv%*%Xmat[,,i]
    }
    V_new = V_new/(pm*n)
    
    epsilonU = sum((U_new - U)^2)
    epsilonV = sum((V_new - V)^2)
    print(c(epsilonU, epsilonV))
    if(epsilonU < 0.01 && epsilonV < 0.01){
      V = V_new
      U = U_new
      #print("yes")
      break
    }
    V = V_new
    U = U_new
  }
  
  m_vec = c(X_mean)
  Cov = kronecker(V,U)
  res = list(m_vec = m_vec, Cov = Cov)
  return(res)
}
#

#symmetric Kullback-Leibler (KL) diver- gence
#Sigma_inv = ginv(Sigma^{-1})
sym_KL <- function(mu,Sigma_inv, Xmat1_vec, Xmat2_vec){
  #term1 = Xmat1_vec - mu
  #term2 = Xmat2_vec - mu
  #res = 0.5*term1%*%Sigma_inv%*%term1 +0.5*term2%*%Sigma_inv%*%term2 - 2*0.5 * length(mu)
  #try gaussian kernal
  #res  = sum((Xmat1_vec- Xmat2_vec)^2)
  #因为我们用的统一Sigma_inv,均值的话用自身来当估计
  term = Xmat1_vec - Xmat2_vec
  res =  t(term)%*%Sigma_inv%*%term
  
  return(res)
}

# set.seed(100)
# y = rnorm(400)
# Xmat = array(runif(prod(c(64,64,400))),c(64,64,400))
# Xmat_list = list()
# Xmat_list[[1]] = Xmat
# Xmat_list[[2]] = Xmat
# 
# Xmat_pre = array(runif(prod(c(64,64,4))),c(64,64,4))
# Xmat_list_pre = list()
# Xmat_list_pre[[1]] = Xmat_pre
# Xmat_list_pre[[2]] = Xmat_pre


obj_fun <- function(y,sigma,Ky){
  n = length(y)
  Kysigma = Ky + sigma^2*diag(n)
  tem=determinant(Kysigma)
  obj = - 0.5*tem$modulus[1] - 0.5*t(y) %*% ginv(Kysigma) %*%y
  return(obj)
}




#为了加速运算，不要一个一个的算KL距离，算整体的距离
KXX_base_fun <- function(Xmat_list, theta){
  
  D = length(theta)-1
  n = dim(Xmat_list[[1]])[3]
  KXX_base = array(0, c(n, n, D))
  res = list()
  uuu = list()
  cov_inverse = list()
  for(d in 1:D){
    res[[d]] = VU_est(Xmat_list[[d]]) 
    uuu[[d]] = res[[d]]$m_vec
    cov_inverse[[d]] = chol2inv(chol(res[[d]]$Cov)) #ginv(res[[d]]$Cov) # 这里有个求逆，很费时间
  }
  
  ppp = prod(dim(Xmat_list[[1]][,,1]))
  
  for(d in 1:D){
    temmatrix = matrix(0,(1+n)/2 *n, ppp)
    for(i in 1:n){
      for(j in 1:i){
        temmatrix[(i-1+1)/2*(i-1)+j,] = c(Xmat_list[[d]][,,i]) - c(Xmat_list[[d]][,,j])
        #print(c(i,j))
        #KXX_base[i,j,d] = 2*kldiv(Xmat[,,i],Xmat[,,j])
        #KXX_base[i,j,d] = sym_KL(uuu[[d]], cov_inverse[[d]],c(Xmat_list[[d]][,,i]),c(Xmat_list[[d]][,,j]))
      }
    }
    print("over")
    temmatrix2 = crossprod(t(temmatrix), cov_inverse[[d]]) * temmatrix
    for(i in 1:n){
      for(j in 1:i){
        print(c(i,j))
        tem = sum(temmatrix[(i-1+1)/2*(i-1)+j,])
        KXX_base[i, j, d] = tem
        KXX_base[j, i, d] = tem
      }
    }
    
  }
  return(KXX_base) 
}



KXX_base_fun_pre <- function(Xmat_list_pre, Xmat_list, theta){
  
  D = length(theta)-1
  n_pre = dim(Xmat_list_pre[[1]])[3]
  n = dim(Xmat_list[[1]])[3]
  KXX_base_pre = array(0, c(n_pre, n, D))
  res = list()
  uuu = list()
  cov_inverse = list()
  for(d in 1:D){
    res[[d]] = VU_est(Xmat_list[[d]]) 
    uuu[[d]] = res[[d]]$m_vec
    cov_inverse[[d]] = chol2inv(chol(res[[d]]$Cov)) #ginv(res[[d]]$Cov) # 这里有个求逆，很费时间
  }
  
  
  ppp = prod(dim(Xmat_list[[1]][,,1]))
  
  for(d in 1:D){
    temmatrix = matrix(0,n_pre *n, ppp)
    for(i in 1:n_pre){
      for(j in 1:n){
        temmatrix[(i-1+1)/2*(i-1)+j,] = c(Xmat_list_pre[[d]][,,i]) - c(Xmat_list[[d]][,,j])
        #print(c(i,j))
        #KXX_base[i,j,d] = 2*kldiv(Xmat[,,i],Xmat[,,j])
        #KXX_base[i,j,d] = sym_KL(uuu[[d]], cov_inverse[[d]],c(Xmat_list[[d]][,,i]),c(Xmat_list[[d]][,,j]))
      }
    }
    print("over")
    temmatrix2 = crossprod(t(temmatrix), cov_inverse[[d]]) * temmatrix
    for(i in 1:n_pre){
      for(j in 1:n){
        print(c(i,j))
        tem = sum(temmatrix[(i-1+1)/2*(i-1)+j,])
        KXX_base_pre[i, j, d] = tem
      }
    }
    
  }
  return(KXX_base_pre) 
}


kernal_fun <- function(Xmat_list,theta, KXX_base){
  alpha = theta[1]
  betam = theta[-1]
  D = length(theta)-1
  
  #exp 里面的分子 ，包括-1/2
  #KXX_base = KXX_base_fun(Xmat_list,theta)
  n = dim(Xmat_list[[1]])[3]
  KXX_final = matrix(0,n,n)
  
  for(d in 1:D){
    KXX_final =   KXX_final  -0.5*KXX_base[,,d]/betam[d]^2
  }
  KXX_final = alpha^(2*D)*exp(KXX_final)
  return(KXX_final)
}


kernal_fun_pre <- function(Xmat_list_pre,Xmat_list,theta, KXX_base_pre){
  alpha = theta[1]
  betam = theta[-1]
  D = length(theta)-1
  
  #exp 里面的分子 ，包括-1/2
  #KXX_base = KXX_base_fun(Xmat_list,theta)
  n_pre = dim(Xmat_list_pre[[1]])[3]
  n = dim(Xmat_list[[1]])[3]
  KXX_final = matrix(0,n_pre,n)
  
  for(d in 1:D){
    KXX_final =   KXX_final  -0.5*KXX_base_pre[,,d]/betam[d]^2
  }
  KXX_final = alpha^(2*D)*exp(KXX_final)
  return(KXX_final)
}


#theta的导数，不包含sigma 
grad_kernal_fun<-function(Xmat_list,theta,KXX_base){
  alpha = theta[1]
  betam = theta[-1]
  D = length(theta)-1
  
  #alpha_all =alpha^(2*D)
  #exp 里面的分子, 不包括-1/2betam
  #D+1的原因是, 1个alpha, D个betam
  #alpha 在第一个位置，betam在后面的D个位置
  #KXX_base = KXX_base_fun(Xmat_list,theta)
  n = dim(Xmat_list[[1]])[3]
  KXX_grad = array(0,c(n,n,D+1))
  KXX_final = matrix(0,n,n)
  for(d in 1:D){
    KXX_final =   KXX_final - 0.5*KXX_base[,,d]/betam[d]^2
  }
  #KXX_final = \sum_{m=1}^M (-\frac{Cm}{2\bbeta_m^2})
  
  KXX_grad[,,1] = 2*D * alpha^(2*D-1) * exp(KXX_final)
  
  for(d in 1:D){
    KXX_grad[,,d+1] = alpha^(2*D)*exp(KXX_final)*(KXX_base[,,d]/betam[d]^3 ) 
  }
  
  #KXX_grad[,,D+2]=diag(n)
  return(KXX_grad)
}

grad_par <- function(Xmat_list,Xmat_list_pre,theta,y,y_pre,sigma,KXX_base,KXX_base_pre){
  
  n = length(y)
  grad_mat = grad_kernal_fun(Xmat_list,theta,KXX_base)
  Ky = kernal_fun(Xmat_list,theta,KXX_base) 
  Ky_inv = ginv( Ky + sigma^2 * diag(n))
  
  
  Kysigma = Ky + sigma^2*diag(n) 
  tem1= -2 * sigma*sum(diag(Ky_inv))+  2*sigma*sum(crossprod(y, Ky_inv)^2)
  #tem2=-sum(diag(2 * sigma *diag(n) %*% ginv(Kysigma)))+t(y)%*%ginv(Kysigma)%*%(2*sigma*diag(n))%*%ginv(Kysigma)%*%y
  
  grad_theta = theta
  D = length(theta)-1
  for(d in 1:(D+1)){
    tem = crossprod(t(Ky_inv), grad_mat[,,d])
    #grad_theta[d] = t(y) %*% tem %*% Ky_inv %*% y - sum(diag(tem))
    grad_theta[d] =  crossprod(t(crossprod(y, tem)), Ky_inv) %*%y- sum(diag(tem))
    #t(y) %*% tem %*% Ky_inv %*% y - sum(diag(tem))
  }
  grad_theta = 0.5 * grad_theta
  Ky_pre = kernal_fun_pre(Xmat_list_pre,Xmat_list,theta,KXX_base_pre) 
  y_pre_k = crossprod(t(Ky_pre), Ky_inv) %*%y 
  print(y_pre_k)
  mspe = sum((y_pre - y_pre_k)^2)/length(y_pre)
  
  #print(c(grad_theta,tem1,obj_fun(y,sigma,Ky),mspe))
  return(c(grad_theta,tem1,obj_fun(y,sigma,Ky),mspe))
}



# KXX_base_use = KXX_base_fun(Xmat_list, c(1,1,1))
# KXX_base_pre_use = KXX_base_fun_pre(Xmat_list_pre, Xmat_list, c(1,1,1))
# 
# final_function(Xmat_list,Xmat_list_pre, y,y_pre, KXX_base_use, KXX_base_pre_use)

final_function <-function(Xmat_list,Xmat_list_pre, y,y_pre, KXX_base_use, KXX_base_pre_use){

res_final = c()

par1 = 10^(-10:3)
par2 = 10^(-2:5)
par3 = 10^(-2:5)
par4 = 10^(-2:3)
n1 = length(par1)
n2 = length(par2)
n3 = length(par3)
n4 = length(par4)

# KXX_base_use = KXX_base
# KXX_base_pre_use = temtem
res_all = matrix(0,n1*n2*n3*n4,10)

for(i1 in 1:n1){
  for(i2 in 1:n2){
    for(i3 in 1:n3){
      for(i4 in 1:n4){
          theta_use = c(par1[i1],par2[i2],par3[i3])
          sigma_use = par4[i4]
        
          res_tem = try(grad_par(Xmat_list,Xmat_list_pre,theta_use,y,y_pre, sigma_use,KXX_base_use, KXX_base_pre_use))
          if("try-error" %in% class(res_tem)){
               res = c(0,0,0,0,-Inf,Inf)
          }else{
                res =res_tem
          }
           index = (i1-1)*(n2*n3*n4) + (i2-1)*(n3*n4) + (i3-1)*n4 + i4
           res_all[index, 1:6] = res
           res_all[index, 7:10] = c(theta_use, sigma_use)
      }
    }
  }
}

tem_index_obj = which.max(res_all[,5])
tem_obj = res_all[tem_index_obj, ] 

tem_index_pre = which.min(res_all[,6])
tem_pre = res_all[tem_index_pre, ] 

res_final = rbind(res_final, tem_obj,tem_pre)

tem = tem_obj
used_scale = c(0.8,0.9,1,1.1,1.2)
par1 = used_scale * tem[7]
par2 = used_scale * tem[8]
par3 = used_scale * tem[9]
par4 = used_scale * tem[10]
n1 = length(par1)
n2 = length(par2)
n3 = length(par3)
n4 = length(par4)
res_all = matrix(0,n1*n2*n3*n4,10)

for(i1 in 1:n1){
  for(i2 in 1:n2){
    for(i3 in 1:n3){
      for(i4 in 1:n4){
        theta_use = c(par1[i1],par2[i2],par3[i3])
        sigma_use = par4[i4]
        
        res_tem = try(grad_par(Xmat_list,Xmat_list_pre,theta_use,y,y_pre, sigma_use,KXX_base_use, KXX_base_pre_use))
        if("try-error" %in% class(res_tem)){
          res = c(0,0,0,0,-Inf,Inf)
        }else{
          res =res_tem
        }
        index = (i1-1)*(n2*n3*n4) + (i2-1)*(n3*n4) + (i3-1)*n4 + i4
        res_all[index, 1:6] = res
        res_all[index, 7:10] = c(theta_use, sigma_use)
      }
    }
  }
}

res_all_obj = res_all

tem = tem_pre
used_scale = c(0.8,0.9,1,1.1,1.2)
par1 = used_scale * tem[7]
par2 = used_scale * tem[8]
par3 = used_scale * tem[9]
par4 = used_scale * tem[10]
n1 = length(par1)
n2 = length(par2)
n3 = length(par3)
n4 = length(par4)
res_all = matrix(0,n1*n2*n3*n4,10)

for(i1 in 1:n1){
  for(i2 in 1:n2){
    for(i3 in 1:n3){
      for(i4 in 1:n4){
        theta_use = c(par1[i1],par2[i2],par3[i3])
        sigma_use = par4[i4]
        
        res_tem = try(grad_par(Xmat_list,Xmat_list_pre,theta_use,y,y_pre, sigma_use,KXX_base_use, KXX_base_pre_use))
        if("try-error" %in% class(res_tem)){
          res = c(0,0,0,0,-Inf,Inf)
        }else{
          res =res_tem
        }
        index = (i1-1)*(n2*n3*n4) + (i2-1)*(n3*n4) + (i3-1)*n4 + i4
        res_all[index, 1:6] = res
        res_all[index, 7:10] = c(theta_use, sigma_use)
      }
    }
  }
}
res_all_pre = res_all

res_all = res_all_obj
tem_index_obj = which.max(res_all[,5])
tem_obj = res_all[tem_index_obj, ] 

res_all = res_all_pre
tem_index_pre = which.min(res_all[,6])
tem_pre = res_all[tem_index_pre, ] 

res_final = rbind(res_final, tem_obj,tem_pre)

return(res_final)
}





rm(X_data_SNR)
rm(X_datacross_reg_SNR)
gc()
