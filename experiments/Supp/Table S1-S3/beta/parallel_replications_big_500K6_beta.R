...
wrapper <- function(idx) {
  ...
  X_data = array(rbeta(64*64*1000, 2, 2),c(64,64,n))
  ...
  for(signal_i in 1:5){
    ...
    res=validation_broadcasted_sparsetenreg(R,alpha,lambda,X_data[,,id_train],y_all[[signal_i]][id_train],X_data[,,id_vali],y_all[[signal_i]][id_vali],X_data[,,id_test],y_all[[signal_i]][id_test],num_knots=5)
...
sfSource('./ParallelComput/parallel_source_500_beta.R')
...
sfStop()
filename <- str_c("BNTR", n_use, "_beta_K6.Rdata")
setwd("./SimResults")
