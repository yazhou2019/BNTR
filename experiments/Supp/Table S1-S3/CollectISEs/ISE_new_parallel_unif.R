...
wrapper <- function(idx) {
  ...
  for(samplesize_i in 1:4){
    ...
    if(n==250){
      path1 = "/data/yzhou/BNTR/FullSimPaper/SimResults/BNTR250_unif_newformat.Rdata"
    }
    if(n==500){
      path1 = "/data/yzhou/BNTR/FullSimPaper/SimResults/BNTR500_unif_newformat.Rdata"
    }
    if(n==750){
      path1 = "/data/yzhou/BNTR/FullSimPaper/SimResults/BNTR750_unif_newformat.Rdata"
    }
    if(n==1000){
      path1 = "/data/yzhou/BNTR/FullSimPaper/SimResults/BNTR1000_unif_newformat.Rdata"
    }
    
    load(path1)
    set.seed(idx*100)
    #X = array(rtnorm(64*64*1000, mean=0.5, sd=0.25, lower=0, upper=1),c(64,64,1000))
    X=array(runif(64*64*1000,0,1),c(64,64,1000))
...
sfStop()
filename <- str_c("ise_meta_unif_n250_500_750_1000.Rdata")
