...
#source('~/Desktop/JRSSBreview2/upload_JRSSB_2nd_respond/NewResults/additional_utility.R')
source('./additional_utility.R')
#True signal 
source('./SimDataGeneration/true_signal_different.R')
source('./SimDataGeneration/true_signal_different_butterfly.R', echo=TRUE)
index = 2

color_used = c(5/255,120/255,5/255)
#color_used = c(5/255,100/255,5/255)
scale_par = 0.8

BB = list()
for(signal_i in 1:5){
  BB[[signal_i]]= ftrue_demean_square_int_ten(case =signal_i)^0.5
}
...
for(signal_i in 1:length(BB)){
  ...
  BBplot_true[,,1] = color_used[1]  * ((-1/color_used[1] + 1) *BB[[signal_i]]/scale_par + 1/color_used[1])
  BBplot_true[,,2] = color_used[2]  * ((-1/color_used[2] + 1) *BB[[signal_i]]/scale_par+ 1/color_used[2]) 
  BBplot_true[,,3] = color_used[3] * ((-1/color_used[3] + 1) *BB[[signal_i]]/scale_par + 1/color_used[3])
