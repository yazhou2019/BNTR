...
# generate \phi(X)
#' @title tildePhiX_trans
#' @description  transform the input \eqn{X} to \eqn{\tilde{\phi}(X)} (truncated power basis without the constant term)
#' @param X_sample The training input without basis transformation.
#' @param knots the knots of the spline basis
#' @param order the order of the spline basis
#' @return
#' \item{BX}{\eqn{\tilde{\phi}(X)} (truncated power basis without the constant term)}
...
#' @title linearpart_penalty_function
#' @description  some function used for debug
#' @references  Y. Zhou, R. K. W. Wong and K. He. Broadcasted Nonparametric Tensor Regression
...
#' @title loglike
#' @description  the loss function; for guassian, it is l2 loss
#' @param y all the response
#' @param linearpart the linear predictor
#' @return the loss
#' @references  Y. Zhou, R. K. W. Wong and K. He. Broadcasted Nonparametric Tensor Regression
...
#' @title full_R
#' @description  transform the tensor of CP decomponets form to the array form
#' @param Beta the tensor in CP components form. Here the CP components hold in an list. Each component takes one slot.
#' @return the tensor in array form
...
#' @title arrange
#' @description  absorb a scale \eqn{a} to the tensor in CP components
#' @param beta the tensor in CP components form.
#' @param a is the scale.
#' @return the absorbed CP components. (list)
...
#' @title elastic_gradient_d
#' @description the rescale factor for elastic this is used for Newton method in rescale strategy
...
# f_d(x)
#' @title elastic_f_lambda_d
#' @description the function used in rescale strategy
...
#' @title elastic_newton_r
#' @description the function used in rescale strategy; Newton's method for r return the lagrange factor
...
#' @title linspace
#' @description  follow the function used in MATLAB toolbox TensorReg
#' @references  Zhou, H., Li, L. and Zhu, H. (2013) Tensor regression with applications in neuroimaging data analysis. Journal of the American Statistical Association, 108, 540–552. (https://hua-zhou.github.io/TensorReg/)
...
#' @title dualascentforalpha
#' @description  dual ascent method to update the coefficients of basis direction.
...
#' @title interpoly_int
#' @description calculate \eqn{\int (x-t1)^m1 (x-t2)^m2 dx}
#' @param t1 one knot
#' @param t2 one knot
#' @param m1 one order
#' @param m2 one order
#' @param right_end the largest knot.
#' @return \eqn{\int (x-t1)^m1 (x-t2)^m2 dx}
...
#' @title fhat_int
#' @description calculate \eqn{\int fhat (entry-wise)}
#' @param coef the coefficient of truncated power basis (without constant term)
#' @param knots the knots of the spline basis
#' @param order the order of the spline basis
#' @return \eqn{\int fhat (entry-wise)}
...
#' @title fhatsqure_int
#' @description calculate \eqn{\int fhat^2 (entry-wize)}
#' @param coef the coefficient of truncated power basis (without constant term)
#' @param knots the knots of the spline basis
#' @param order the order of the spline basis
#' @return \eqn{\int fhat^2 (entry-wize)}
...
#' @title fhatnorm_ten
#' @description calculate the norm tensor for BroadcasTR
#' @param knots the knots of the spline basis
#' @param order the order of the spline basis
#' @param BB the estimated coeffcient tensor in array form.
#' @return the norm tensor
#' @references  Y. Zhou, R. K. W. Wong and K. He. Broadcasted Nonparametric Tensor Regression
...
# generate one truncated Spline (not use this due to speed, can be used for debug)
#' @title truncatS
#' @description generate one truncated Spline; used for debug.
#' @param knots the knots of the spline basis
#' @param order the order of the spline basis
#' @param x the point
#' @return  truncated power basis function value without constant term
#' @references  Y. Zhou, R. K. W. Wong and K. He. Broadcasted Nonparametric Tensor Regression
...
#' @title ctprod
#' @description contract inner product of two tensor; developed in other package.
#' @param A a tensor
#' @param B a tensor
#' @param K the inner product \eqn{< >_K}
#' @return  contract inner product
#' @references  Lock, E. F. (2018) Tensor-on-tensor regression. Journal of Computational and Graphical Statis- tics, 27, 638–647. https://github.com/lockEF/MultiwayRegression.
